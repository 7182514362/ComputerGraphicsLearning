...
void Renderer::Render(const Scene &scene)
{
    ...
    // 缩放参数
    float z = -1.0f;
    float scale_y = (-z) * std::tan(deg2rad(scene.fov * 0.5f));
    float imageAspectRatio = scene.width / (float)scene.height;
    float scale_x = (-z) * scale_y * imageAspectRatio;
    ...
    for (int j = 0; j < scene.height; ++j) {
        for (int i = 0; i < scene.width; ++i) {
            ...
            x = (2.0f * ((float)i / (float)scene.width) - 1.0f) * scale_x;

            y = (1.0f - 2 * ((float)j / (float)scene.height)) * scale_y;

...
GLuint OSDText::compile_shaders()
{
    ...
    if (info_length > 0) {
        ...
        glGetShaderInfoLog(vert_shader, info_length, NULL,
    ...
    if (info_length > 0) {
        ...
        glGetShaderInfoLog(frag_shader, info_length, NULL,
...
GLint OSDText::get_attribu(GLuint program, const char *name)
{
    ...
    if (attribute == -1) fprintf(stderr, "Cannot bind attribute %s\n", name);
    return attribute;
}
...
GLint OSDText::get_uniform(GLuint program, const char *name)
{
    ...
    if (uniform == -1) fprintf(stderr, "Cannot bind uniform %s\n", name);
    return uniform;
}
